...
void main() {
  ...
  test('Right', () {
    final where0 = Where.eq(userAddress, addrId);
    final join = Join.right(address, [where0]);
    expect(
  ...
  test('Inner', () {
    final where0 = Where.eq(userAddress, addrId);
    final join = Join.inner(address, [where0]);
    expect(
  ...
  test('Full', () {
    final where0 = Where.eq(userAddress, addrId);
    final join = Join.full(address, [where0]);
    expect(join.sql(), 'FULL JOIN address AS addr ON (user.address = addr.id)');
  });

  test('Cross', () {
    final where0 = Where.eq(userAddress, addrId);
    final join = Join.cross(address, [where0]);
    expect(
  ...
  test('With Query', () {
    ...
    final join = Join.left(sq, [where0], alias: 'p');
    expect(join.sql(),
